...
export default function TeamDetailPage() {
  ...
  async function load(currentTeamId: string) {
    ...
    try {
      ...
      const [teamRes, membersData, findingsData] = await Promise.all([
        fetch(`${API_BASE_URL}/api/teams/${currentTeamId}`, {
          headers: { Authorization: `Bearer ${session.access_token}` },
        }).then((r) => (r.ok ? r.json() : Promise.reject(r.statusText))),
        getTeamMembers(currentTeamId),
        getTeamFindings(currentTeamId),
      ])
  ...
  findings.forEach((f) => {
    moduleMap[f.module] = (moduleMap[f.module] ?? 0) + 1
  })
  ...
  if (error || !team) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4">
        ...
        <button onClick={() => navigate('/team')} className="btn-secondary" type="button">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            {team.team_code && (
              <p className="text-xs text-brand-400 font-mono mt-1">Team ID: {team.team_code}</p>
            )}
        ...
        {isLeader && (
          <div className="flex items-center gap-2 flex-wrap">
            <Link to={`/team/${teamId}/policies`} className="btn-secondary flex-shrink-0">
              <ShieldCheck className="w-4 h-4" /> Manage Policies
            </Link>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <span
                        className={`badge ${
                          member.role === 'leader' ? 'badge-info' : 'badge-low'
                        }`}
                      >
                        {member.role}
                      </span>
                  ...
                  <div className="grid grid-cols-4 gap-1 text-center">
                    {[
                      ...
                      {
                        label: 'Critical',
                        value: member.stats?.critical ?? 0,
                        color: 'text-red-400',
                      },
                      { label: 'High', value: member.stats?.high ?? 0, color: 'text-orange-400' },
                      {
                        label: 'Med/Low',
                        value: (member.stats?.medium ?? 0) + (member.stats?.low ?? 0),
        ...
        <div className="card">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {moduleData.map(({ name, value }) => (
              <div
                key={name}
                className="bg-surface-tertiary rounded-xl p-4 text-center border border-surface-border"
              >
                <div
                ...
                >
                  <span
                    ...
                    style={{ color: MODULE_COLORS[name] ?? '#6b7280' }}
                  >
                    {name}
                  </span>
                </div>
        ...
        <div className="card">
          ...
          <div className="space-y-2">
            {topFindings.map((f) => (
              <div
                key={f.id}
                className="flex items-center gap-3 p-3 rounded-lg bg-surface-tertiary border border-surface-border"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {f.file_path && (
                    <p className="text-xs text-gray-500 font-mono truncate">{f.file_path}</p>
                  )}
                </div>

                <span className={`chip-${f.module?.toLowerCase()} flex-shrink-0 text-xs`}>
                  {f.module}
                </span>
              </div>
            ))}
